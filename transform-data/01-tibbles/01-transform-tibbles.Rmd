...
In the tutorials, we'll use a dataset named `luquillo_stem_random`, which comes in the package __fgeo.data__. Within `luquillo_stem_random`, you will find information about a subset of randomly selected trees recorded in census 5 of the forest plot Luquillo, in Puerto Rico.

This tutorial introduces `luquillo_stem_random` as well as a new data structure that makes working with data in R easy: the tibble.
...
What if you'd like to inspect the remaining portions of a tibble? To see the entire tibble, use the `View()` command. R will launch a window that shows a scroll-able display of the entire data set. Try this below, run `View(luquillo_stem_random)` to see the entire data set.

```{r babynames-4, exercise = TRUE, exercise.setup = "bnames"}

```

```{r babynames-4-solution}
