...
stem_count %>% 
  ...
  geom_line() +
  ggtitle(glue("The history of {spp}"))
```
...
stem_count %>% 
  ...
  ggplot(aes(CensusID, n)) + 
  geom_point() +
  labs(title = glue("The history of {spp}"))
```
...
Your goal in this section is to repeat this process for `r spp` (or a species that you choose). Along the way, you will learn a set of functions that isolate information within a data set. 
